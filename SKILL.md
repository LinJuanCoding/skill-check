...
4. **报告标注**  
   - 审查模式写 **「安装前远程审查（未安装）」** 或 **「本地已安装」**  
   - 记录审查时间  
   - 结论里区分「静态审查通过」与「是否建议安装」
...
- **目标**只写用户给的一个地址，Git URL 或本地路径，**不要**再拼接 `/ skill-name`
- **不写 commit**，报告里不出现 commit SHA
- **审查模式**只写「本地已安装」或「安装前远程审查（未安装）」，不加括号补充说明
